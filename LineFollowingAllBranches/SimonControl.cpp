...
void SimonControl::Grab()
{
	...
	parent->Rear_Smooth_Move(1, 0, 2, gripAngle, 95, -30, 8.5, 3, -2.3, gripAngle, 100, -90, 1);
	parent->Rear_Smooth_Move(8.5, 3, -2.3, gripAngle, 100, -90, 7.1, 3, -2.6, gripAngle, 95, -100, 1);
...
{
	double gripAngle = 70;
	parent->Front_Smooth_Move(-5.2, 1.5, -2, 175, 90, -80, -4, 0, -2, 90, 90, -80, 1); //move arm to the right
	parent->Front_Smooth_Move(-4, 0, -2, 175, 90, -80, -1, 0, 2, 90, 90, -30, 1); //move arm to home
	parent->Front_Smooth_Move(-1, 0, 2, 90, 90, -30, -1, 0, 2, 90, 90, 90, 1); //move wrist out of the way
	parent->Rear_Smooth_Move(7.1, 2.7, -2.6, gripAngle, 90, -100, 4.6, 2.7, -1,gripAngle,110,-110,1); //comes in
	parent->Rear_Smooth_Move(4.6, 2.7, -1, gripAngle, 110, -110, 4.6, 2.7, -2, gripAngle, 110, -110, 1); //stick it to skid
	parent->Rear_Smooth_Move(4.6, 2.7,-1,gripAngle,110,-110, 4.6, 2.3,-1,0,110,-110,1); //lets go
	parent->Rear_Smooth_Move(4.6, 2.3, -1, 0, 110, -110, 3.4,2.3,0,0,110,-110,1); //moves above hat
	parent->Rear_Smooth_Move(3.4,2.3,0,0,110,-110, 1,0,2,90,90,-30,1); //goes home
	parent->Front_Smooth_Move(-1, 0, 2, 90, 90, 90, -1, 0, 2, 90, 90, -30, 1); //goes home
	//flip to other side
	parent->RearHomeLeft();
