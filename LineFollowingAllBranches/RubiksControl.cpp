...
// Squeeze together
	...
	double B[6]={8.5, 2, -4.3, GripConstant, 92, -105};
	...
	// Pull out
	double A2[6] = { 8.5, 2, -4.3, GripConstant, 92, -105 };
	...
	parent->Rear_Smooth_Move(8.5, 3, -4, GripConstant, 100, 0, 8.5, 3.2, -4, GripConstant, 115, -100, .8);
	delay(100); 

	parent->Rear_Smooth_Move(8.5, 3.2, -4, GripConstant, 115, -100,    6.7, 3.2, -4.5, GripConstant, 115, -110,.8); 
	delay(1000);
	

	// move out of the way
	parent->Rear_Smooth_Move( 6.7, 3.2, -4.5, GripConstant, 118, -110,    9, 3.2, -2, GripConstant, 115, -90, TimeConstant);
	delay(100);

	//set hat straight
	parent->Rear_Smooth_Move(9, 3.2, -2, GripConstant, 115, -90, 8.9, 3.2, -5.7, GripConstant, 115, -110, TimeConstant);
	delay(1000);

	//Intermediate Step
	parent->Rear_Smooth_Move(9, 3.2, -2, GripConstant, 115, -90, 6, 2.9, 0, GripConstant, 33, -105, TimeConstant);
	delay(100); 
...
void RubiksControl::Rotate()
{
	...
	// place hat on it
	double GripConstant = 100;
	parent->Rear_Smooth_Move(4.5, 2.9, -2, GripConstant, 33, -105,                        4.5, 2.5, -2.7, GripConstant, 33, -105,                      TimeConstant); 
	delay(100);

	// Fudge factoring savannah changed -2.5 to -2.8
	parent->Rear_Smooth_Move( 4.5, 3.1, -2.8, GripConstant, 33, -105,                          4.5, 2.3, -2.3, GripConstant, 33, -105,                        TimeConstant); //really make sure hat is there
	delay(100);
	...
	delay(100);
		...
		delay(100);
		parent->Rear_Smooth_Move(4.5, 2.6, -2.5, GripConstant, 140, -105, 4.5, 2.6, -2, GripConstant, 140, -105, TimeConstant); //move up 4.5
		delay(100);
		parent->Rear_Smooth_Move(4.5, 2.6, -2, GripConstant, 140, -105, 4.5, 2.6, -2.2, GripConstant, 33, -105, TimeConstant); //reset wrist
		delay(100);
		parent->Rear_Smooth_Move(4.5, 3.1, -2.8, GripConstant, 33, -105, 4.5, 2.3, -2.3, GripConstant, 33, -105, TimeConstant); //really make sure hat is there
		delay(100);
		parent->Rear_Smooth_Move(4.5, 2.5, -2.8, GripConstant, 33, -105, 4.9, 2.5, -2.8, GripConstant, 33, -105, TimeConstant); //really make sure hat is there
